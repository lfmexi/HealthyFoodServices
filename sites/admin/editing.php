                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-6">
								<?php
								if (!isset($_POST['name'])&&!isset($_POST['cal'])&&isset($_POST['id'])){
								...
								$result1 = mysql_query($query);
                                    ...
                                    echo '    </div>';
                                    echo '    <div class="form-group">';
                                    echo '        <label>Calorias:</label>';
                                    echo '        <input class="form-control" name="cal" value="'.$row["cantidad"].'">';
                                    echo '    </div>';
                                    echo '    <input type="hidden" name="id" value="'.$id.'">';
                                    echo '    <button type="Ingresar" class="btn btn-default">Editar</button>';
                                    echo '    <button type="" class="btn btn-default">Reset Button</button>';
                                    echo '</form>';
}
								}else if (isset($_POST['name'])&&isset($_POST['cal'])&&isset($_POST['id'])){
                                                                $conn = mysql_connect("mysql.hostinger.es","u147283082_admin" ,"seminario2");
								mysql_selectdb("u147283082_hlife", $conn);
								$name=$_POST['name'];
								$cal=$_POST['cal'];
								$id=$_POST['id'];
								$query = "UPDATE Ingrediente SET Nombre='".$name."', cantidad='".$cal."' WHERE id=".$id.";";
								//mysql_query("BEGIN");
								$result1 = mysql_query($query);
								if (!$result1){
									echo "El ingrediente no fue actualizado";
								}else{
									echo "El ingrediente fue actualizado con éxito";
								}
								//mysql_query("COMMIT");	
								mysql_close($conn);
								}
